...
'''
import kernel.event
import stackless,sys,traceback
import cursors
...
def defaulterrorhandler(connection, cursor, errorclass, errorvalue):
    ...
    if not connection.open: 
        kernel.event.UnBind(connection.fd)
    #----
...
class Connection(_mysql.connection):
    ...
    def __del__(self):
        kernel.event.UnBind(self._myfd)
    ...
    def _release_wait(self,_id,err=0,bInitial = False):
        ...
        def _f():
            ...
            pass
        #
        if bInitial:    stackless.tasklet(_f)().insert(); return 
        if self._waitings.pop(_id,False):
            stackless.tasklet(_f)().insert()
    ...
    def _on_sock_write(self,obj_arg):
        #print '_on_sock_write'
        pass
        
    def _on_sock_error(self,obj_arg):
        print '_on_sock_error'
    ...
    def _event_cb(self,fd,evflags,obj_arg):
        if evflags & kernel.event.EVENT_READ:     self._on_sock_read(obj_arg)
        elif evflags & kernel.event.EVENT_WRITE:  self._on_sock_write(obj_arg)
        elif evflags & kernel.event.EVENT_ERROR:  self._on_sock_error(obj_arg)
        #print "fd:%s    event-flags:%s   obj_arg:%s" % (fd,evflags,obj_arg)
